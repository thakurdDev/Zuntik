...
#import "NewEventVC.h"
@interface MycalendarEventVC ()<DSLCalendarViewDelegate>
{
...
@implementation MycalendarEventVC
@synthesize arrForResponse;
@synthesize strCal_key;
...
- (void)viewDidLoad
{
    ...
    lblHeader.text=[NSString stringWithFormat:@"Calendar: %@",[strCal_key objectForKey:@"cal_key"]];
...
-(void)CheckNetwork  //Network Check(1)
{
    ...
    {
        if ([[strCal_key objectForKey:@"cal_type"] isEqualToString:@"2"])
        {
            [self ONDataBaseEventsValue];
        }
        else
...
-(void)ONDataBaseEventsValue
{
    ...
    dicResponse= [objDatabase eventInfoList_Calendar:[strCal_key objectForKey:@"cal_key"]]; //Database Method
...
{ [self MBProgressHUDMethod];
    NSDictionary *parameters = @{@"cal_key":[strCal_key objectForKey:@"cal_key"],@"APIAccount":@"ZuntikAppAPIUser",@"APIPassword":@"n4kfoqjgfxjsmujelznss7il6n9d9w4nrfs5w2b1"};
    [ZuntikServicesVC PostMethodWithApiMethod:@"returnCalEvents.php" Withparms:parameters WithSuccess:^(id response)
     {
         ...
         {
             [objDatabase deleteEventsCalsData_Cal_id:[strCal_key objectForKey:@"cal_key"]];  //delete data on database on userid
            self.arrForResponse=[dicResponse valueForKey:@"events"];
...
-(IBAction)AllCal_EventsAction:(id)sender
{
    if ([btncheckValue isSelected])
    {
        ...
        _strTblReload=@"DateEvents";
        btncheckValue.selected=NO;
        [btncheckValue setImage:[UIImage  imageNamed:@"cal_switch.png"] forState:UIControlStateNormal];
    ...
    else
    {
        ...
        [btncheckValue setImage:[UIImage  imageNamed:@"list_switch.png"] forState:UIControlStateNormal];
        
        btncheckValue.selected=YES;
